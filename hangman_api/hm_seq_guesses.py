...
def simulate_incorrect_guesses(word):
    ...
    for _ in range(random.randint(1, 5)):  # Randomly choose 1-5 incorrect guesses
        ...
        if incorrect_char not in word:
            incorrect_guess_vector[char_to_int[incorrect_char]] = 30  # Mark as incorrect guess
...
def preprocess_data(words, max_word_length, p_missing=0.4):
    ...
    targets = pad_sequences(targets, maxlen=max_word_length + 26, padding='post')
...
def train(words):
    ...
    model = Sequential()
    model.add(Embedding(input_dim=len(chars)+4, output_dim=128, trainable=True))
    ...
    early_stopping = EarlyStopping(monitor='val_loss', patience=3, verbose=1, mode='min', restore_best_weights=True)
    checkpoint_filepath = '/lstm6_10_seq_guesses.h5'
