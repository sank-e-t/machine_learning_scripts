...
def simulate_missing_letters(word, char_to_int, p_missing=0.4):
    ...
    less_simulated_word = ''
    for char in word:
        less_simulated_word += '0' if rand_dic[char] < p_missing - 0.2 else char

    return simulated_word, less_simulated_word
...
def preprocess_data(words, max_word_length, p_missing=0.4):
    ...
    for word in words:
        for i in range(3):
            word_with_missing, word_with_less_missing = simulate_missing_letters(word, char_to_int, 0.6-0.2*i)
            sequences.append([char_to_int[char] if char in char_to_int else char_to_int['0'] for char in word_with_missing])
            targets.append([char_to_int[char] for char in word_with_less_missing])
